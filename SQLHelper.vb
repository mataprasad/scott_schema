...
''' <summary>
'''  Title      : SQL Helper 
'''  Description: he SqlHelper class is intended to encapsulate high performance, scalable best practices for common uses of SqlClient.
'''  Copyright  : Pearson India
'''  Company    : HCL Technologies Ltd. Noida. India.
'''  Date       : Aug/01/08
'''  @filename  : SQLHelper.vb
'''  @author    : <a href="mailto:amit.oberoi@hcl.in">Amit Oberoi</a>
''' </summary>
''' <remarks>
''' ****************************  Modification History  *************************************************************
''' Sr No:	Date	   Modified by	    Request/Tracker                 Description
''' *****************************************************************************************************************
''' *****************************************************************************************************************
'''</remarks>
...
Public NotInheritable Class SqlHelper
    Private Shared log As ILogger = LoggerFactory.getLogger()
...
#Region "private utility methods & constructors"
    ...
    Private Shared Sub AssignParameterValues(ByVal commandParameters() As SqlParameter, ByVal parameterValues() As Object)
        ...
        Dim iCmdCnt As Short
        Dim iCmdParamLen As Short
        ...
        'value array
        iCmdParamLen = commandParameters.Length - 1
        For iCmdCnt = 0 To iCmdParamLen
            commandParameters(iCmdCnt).Value = parameterValues(iCmdCnt)
        Next
    ...
    Private Shared Sub PrepareCommand(ByVal command As SqlCommand, _
        ...
        'To set the timeout default value 
        command.CommandTimeout = Utilities.WebConfiguration.SQLCmdTimeOut
    ...
    Private Shared Sub PrepareCommand(ByVal command As SqlCommand, _
                                      ...
                                      ByVal commandType As CommandType, _
                                      ByVal commandText As String, _
                                      ByVal commandTimeout As Int16, _
                                      ByVal commandParameters() As SqlParameter)
...
#Region "ExecuteNonQuery"
    ...
    Public Overloads Shared Function ExecuteNonQuery(ByVal connectionString As String, _
        ...
        Catch sqlEx As SqlException When ((sqlEx.Procedure.ToString() = "ConnectionOpen (Connect()).") Or (sqlEx.Procedure.ToString() = "ConnectionCheckForData (CheckforData()).") Or (sqlEx.Procedure.ToString() = "ConnectionRead (recv()).") Or (sqlEx.Procedure.ToString() = "ConnectionWrite (send())."))
            log.LogException(sqlEx)
            Throw New PIException("3156")
        Catch sqlEx As SqlException When ((sqlEx.Number <> -2) And (sqlEx.Procedure.ToString() <> "ConnectionOpen (Connect())."))
            log.LogException(sqlEx)
            Throw New PIException("3157")
        Catch sqlEx As SqlException
            log.LogException(sqlEx)
            If (sqlEx.Number = -2 OrElse InStr(sqlEx.Message.Trim.ToUpper, "TIMEOUT EXPIRED", CompareMethod.Text) >= 0) Then
                Throw New PIException("3166")
            Else
                Throw New PIException("3158")
            End If
...
#Region "ExecuteDataset"
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connectionString As String, _
        ...
        Try
            cn.Open()
            'call the overload that takes a connection in place of the connection string
            Return ExecuteDataset(cn, commandType, commandText, commandTimeout, commandParameters)
        Catch sqlEx As SqlException When ((sqlEx.Procedure.ToString() = "ConnectionOpen (Connect()).") Or (sqlEx.Procedure.ToString() = "ConnectionCheckForData (CheckforData()).") Or (sqlEx.Procedure.ToString() = "ConnectionRead (recv()).") Or (sqlEx.Procedure.ToString() = "ConnectionWrite (send())."))
            log.LogException(sqlEx)
            Throw New PIException("3156")
        Catch sqlEx As SqlException When ((sqlEx.Number = -2) OrElse (InStr(sqlEx.Message.Trim.ToUpper, "TIMEOUT EXPIRED", CompareMethod.Text) > 0))
            log.LogException(sqlEx)
            Throw New PIException("8009")
        Catch sqlEx As SqlException
            log.LogException(sqlEx)
    ...
    Public Overloads Shared Function ExecuteDataset(ByVal connection As SqlConnection, _
        ...
        PrepareCommand(cmd, connection, CType(Nothing, SqlTransaction), commandType, commandText, commandTimeout, commandParameters)
...
#Region "ExecuteScalar"
    ...
    Public Overloads Shared Function ExecuteScalar(ByVal connectionString As String, _
        ...
        Catch ex As Exception
            ...
            Throw New PIException("3158")
        Finally
            'if we fail or success the Execute Scalar, we need to close the connection ourselves
            cn.Dispose()
        End Try
    End Function 'ExecuteScalar
...
#Region "private methods, variables, and constructors"
    ...
    Private Shared Function CloneParameters(ByVal originalParameters() As SqlParameter) As SqlParameter()
        ...
        Dim iCmdCnt As Short
        Dim iCmdParamLen As Short = originalParameters.Length - 1
        Dim clonedParameters(iCmdParamLen) As SqlParameter

        For iCmdCnt = 0 To iCmdParamLen
            clonedParameters(iCmdCnt) = CType(CType(originalParameters(iCmdCnt), ICloneable).Clone, SqlParameter)
        Next
